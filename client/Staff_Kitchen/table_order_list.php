...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['orderId'], $_POST['newStatus'])) {
    ...
    if ($orderId && in_array($newStatus, ['in_progress', 'complete'])) {
        $stmt = $conn->prepare("UPDATE `order` SET status_kitchen = :status WHERE order_id = :orderId");
...
$whereClause = "WHERE o.status_kitchen = 'in_progress'";
$params = [];
...
$query = "
    ...
    $whereClause
    GROUP BY g.getting_table_id, o.order_id, rm.item_name, o.quantity, o.order_date, o.status_kitchen
                            ...
                            <tr>
                                ...
                                <td>
                                    <select class="status-dropdown" data-order-id="<?= $row['order_id'] ?>">
                                        <option value="in_progress" <?= $row['status_kitchen'] == 'in_progress' ? 'selected' : '' ?>>In Progress</option>
                                        <option value="complete" <?= $row['status_kitchen'] == 'complete' ? 'selected' : '' ?>>Complete</option>
                                    </select>
                                </td>
                            </tr>
                        <?php endforeach; ?>
