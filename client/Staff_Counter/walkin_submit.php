...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Debug: Log incoming POST data to console
    ...
    // ตรวจสอบชื่อ-นามสกุลในระบบสมาชิก
    ...
    if (mysqli_num_rows($check_result) === 0) {
        echo "<script>alert('ไม่พบชื่อและนามสกุลนี้ในระบบสมาชิก'); window.history.back();</script>";
        exit;
    }
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        // 3. บันทึกข้อมูล Walk-in
        $insert_sql = "INSERT INTO walkin (first_name, last_name, number_of_guest, availability_id) 
                       VALUES (?, ?, ?, ?)";
        $stmt_insert = $conn->prepare($insert_sql);
        $stmt_insert->bind_param("ssii", $first_name, $last_name, $number_of_guest, $availability_id);
